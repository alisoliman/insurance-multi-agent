...
function getLabelPos(fromId: string, toId: string) {
  ...
  const len = Math.sqrt(dx * dx + dy * dy) || 1
  const labelOff = 16
  return { labelX: qx + (dy / len) * labelOff, labelY: qy - (dx / len) * labelOff }
}
            ...
            <g key={`${flow.from}-${flow.to}`}>
              {isHighlighted && (
                <motion.path
                  ...
                  strokeWidth={6} strokeLinecap="round" opacity={0.2}
              ...
              <motion.path
                ...
                strokeWidth={isHighlighted ? 2 : 1} strokeLinecap="round"
                strokeDasharray={isHighlighted ? "none" : "4 8"}
                initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 1 }}
                transition={{ delay: 0.3 + i * 0.1, duration: 0.6 }}
              />

              <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 + i * 0.1 }}>
                {(() => {
                  ...
                  return (
                    <>
                      <rect
                        x={labelX - pillW / 2} y={labelY - 9} width={pillW} height={16} rx={8}
                        fill={isHighlighted ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.03)"}
                        stroke={isHighlighted ? "rgba(255,255,255,0.10)" : "none"} strokeWidth={0.5}
                      />
                      <text
                        x={labelX} y={labelY + 3} textAnchor="middle"
                        fill={isHighlighted ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.25)"}
                        fontSize="8.5" fontWeight={isHighlighted ? 500 : 400} letterSpacing="0.03em"
                      >
                        {flow.label}
                      </text>
                    </>
              ...
              {reducedMotion ? (
                <circle
                  ...
                  fill={isHighlighted ? "#fff7ec" : "rgba(255,255,255,0.25)"} opacity={0.6}
                />
              ) : (
                <motion.circle
                  r={isHighlighted ? 3.5 : 2}
                  ...
                  animate={{ cx: [from.x, to.x], cy: [from.y, to.y], opacity: [0, 0.85, 0.85, 0] }}
                  transition={{
        ...
        {architectureModules.map((mod, i) => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.rect
                  x={pos.x - 6} y={pos.y - 6} width={nodeW + 12} height={nodeH + 12} rx={22}
                  fill="none" stroke={primaryColor} strokeWidth={1.5} filter="url(#active-glow)"
                  initial={{ opacity: 0.5 }}
              ...
              <rect
                x={pos.x} y={pos.y} width={nodeW} height={nodeH} rx={16}
                fill={isActive ? `url(#arch-grad-${mod.id})` : "rgba(255,255,255,0.03)"}
                stroke={isActive ? primaryColor : "rgba(255,255,255,0.10)"}
                strokeWidth={isActive ? 1.5 : 1}
              />
              <rect
                x={pos.x + 1} y={pos.y + 1} width={nodeW - 2} height={nodeH - 2} rx={15}
                fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth={0.5}
              />
              <rect
                x={pos.x + 16} y={pos.y + 12}
                width={isActive ? 36 : 24} height={3} rx={1.5}
                fill={primaryColor} opacity={isActive ? 1 : 0.35}
              />
              <text
                x={pos.x + nodeW / 2} y={pos.y + 34}
                textAnchor="middle" fill={isActive ? "#fff7ec" : "#c8d0da"}
                fontSize="13" fontWeight="600" letterSpacing="0.01em"
              >
                {mod.name}
              </text>
              <text
                x={pos.x + nodeW / 2} y={pos.y + 53}
                textAnchor="middle" fill={isActive ? "#9fb0c4" : "#576879"}
