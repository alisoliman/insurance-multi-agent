      ...
      >
        <defs>
          ...
          {agents.map((agent) => {
            ...
            return (
              <linearGradient
                key={`cg-${agent.id}`} id={`cg-${agent.id}`}
                gradientUnits="userSpaceOnUse"
                x1={String(agent.x)} y1={String(agent.y)} x2={String(CX)} y2={String(CY)}
              >
        ...
        {/* Orbit ring */}
        <circle cx={CX} cy={CY} r={ORBIT_R} fill="none" stroke="url(#orbit-grad)" strokeWidth={8} opacity={0.08} filter="url(#cg-glow)" />
        <motion.circle
        ...
        />
        <circle cx={CX} cy={CY} r={ORBIT_R + 10} fill="none" stroke="rgba(255,255,255,0.025)" strokeWidth={0.5} strokeDasharray="2 8" />
        ...
        {agents.map((agent, i) => {
          ...
          return (
            <g key={`conn-${agent.id}`}>
              {isActive && (
                <path d={d} fill="none" stroke={agent.color} strokeWidth={6} opacity={0.12} filter="url(#cg-glow)" strokeLinecap="round" />
              )}
              <motion.path
                d={d} fill="none" stroke={`url(#cg-${agent.id})`}
                strokeWidth={isActive ? 2.5 : 1} strokeLinecap="round"
                initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 1 }}
                transition={{ delay: 0.3 + i * 0.1, duration: 0.55 }}
              />
              {isActive && !reducedMotion && (
                <motion.circle
                  r={3.5} fill={agent.color} filter="url(#cg-glow)"
        ...
        >
          <circle cx={CX} cy={CY} r={CENTER_R + 6} fill={SYNTH_COLOR} opacity={activeId === "synthesizer" ? 0.2 : 0.08} filter="url(#cg-glow-lg)" />
          <circle
          ...
          />
          <text x={CX} y={CY + 1} textAnchor="middle" dominantBaseline="middle" fill="#fff7ec" fontSize="9.5" fontWeight="600" letterSpacing="0.03em">
            Synthesizer
          </text>
        </motion.g>
        ...
        {agents.map((agent, i) => {
          ...
          return (
            ...
            >
              {isActive && (
                <circle cx={agent.x} cy={agent.y} r={NODE_R + 4} fill={agent.color} opacity={0.18} filter="url(#cg-glow)" />
              )}
              {isActive && !reducedMotion && (
                <motion.circle
              ...
              <text
                ...
                fill={isActive ? "#fff7ec" : "#c0c8d4"} fontSize="12" fontWeight="600"
              >
                {agent.name.split(" ").map((w) => w[0]).join("")}
              </text>
              <text
                x={agent.x} y={agent.y + 42}
                textAnchor="middle" fill={isActive ? "#fff7ec" : "#9fb0c4"}
