...
Always provide detailed reasoning, identify specific risk factors, and ensure decisions are explainable and auditable for regulatory compliance."""
        ...
        super().__init__(name="enhanced_assessment_agent", system_message=system_message)
    ...
    async def assess_claim(self, claim_data: Dict[str, Any],
                           policy_data: Optional[Dict[str, Any]] = None) -> AssessmentResult:
        """
        ...
        try:
            ...
            return await self._fallback_assessment(claim_data, str(e), start_time)

    async def _perform_llm_assessment(self, claim_data: Dict[str, Any],
                                      policy_data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _build_assessment_prompt(self, claim_data: Dict[str, Any],
                                 policy_data: Optional[Dict[str, Any]]) -> str:
...
"""
    ...
    def _extract_structured_data_from_text(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'regulatory_compliance': {'compliant': True, 'requirements': [], 'missing_items': []},
            'recommended_actions': ['Review assessment', 'Proceed with standard processing']
        }

    def _apply_business_rules(self, llm_result: Dict[str, Any],
                              claim_data: Dict[str, Any]) -> AssessmentResult:
    ...
    def _create_error_result(self, error_message: str, start_time: datetime) -> AssessmentResult:
        ...
        return AssessmentResult(
            ...
            regulatory_compliance={'compliant': False,
                                   'requirements': [], 'missing_items': []},
            recommended_actions=[
    ...
    async def _fallback_assessment(self, claim_data: Dict[str, Any],
                                   error_message: str, start_time: datetime) -> AssessmentResult:
        """Fallback to rule-based assessment when LLM fails."""
        processing_time = (datetime.now() - start_time).total_seconds()
        ...
        if amount > 50000:
            ...
            decision = AssessmentDecision.INVESTIGATE
            confidence = 0.5
        ...
        return AssessmentResult(
            ...
            reasoning=f"Fallback rule-based assessment due to LLM error: {error_message}",
            risk_factors=[],
            policy_coverage_analysis={'fallback': True},
            fraud_risk_score=0.3,
            documentation_completeness=0.5,
            regulatory_compliance={'compliant': True,
                                   'requirements': [], 'missing_items': []},
            recommended_actions=[
                'Review with enhanced assessment when LLM is available'],
            processing_time_seconds=processing_time,
            assessment_id=str(uuid.uuid4()),
            timestamp=datetime.now()
        )

    async def assess_claim_validity(self, claim_data: Dict[str, Any],
                                    policy_terms: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
