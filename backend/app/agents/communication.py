...
Always ensure accuracy and consistency with provided assessment results."""
    ...
    async def generate_communication(self, context: CommunicationContext) -> CommunicationResult:
        """
        ...
        try:
            ...
            communication_data = await self._perform_llm_communication_generation(context)
            ...
            return CommunicationResult(
                ...
                metadata={
                    ...
                    'llm_response_quality': validated_communication.get('quality_score', 0.0),
                    'compliance_checks_passed': validated_communication.get('compliance_checks', []),
                    'personalization_elements': validated_communication.get('personalization_elements', [])
                }
            )

        except Exception as e:
            # Fallback to template-based generation
            return await self._fallback_communication_generation(context, str(e))

    async def _perform_llm_communication_generation(self, context: CommunicationContext) -> Dict[str, Any]:
        """
        ...
        try:
            # Use AutoGen agent for LLM interaction
...
Generate the communication now:"""
    ...
    def _parse_text_response(self, content: str, context: CommunicationContext) -> Dict[str, Any]:
    ...
    def _apply_compliance_validation(self, communication_data: Dict[str, Any],
        ...
        if context.communication_type in [CommunicationType.REJECTION_NOTIFICATION,
    ...
    def _calculate_personalization_score(self, communication_data: Dict[str, Any],
        ...
        if context.policy_details and context.policy_number in communication_data['content']:
            score += 0.2

        # Check for appropriate tone
        tone_keywords = {
            CommunicationTone.CONGRATULATORY: ['congratulations', 'pleased', 'approved'],
        ...
        expected_tone = self.tone_mapping.get(
            context.communication_type, CommunicationTone.PROFESSIONAL)
        if any(keyword in communication_data['content'].lower()
               for keyword in tone_keywords.get(expected_tone, [])):
            score += 0.1

        return min(score, max_score)

    async def _fallback_communication_generation(self, context: CommunicationContext,
                                                 error: str) -> CommunicationResult:
        """Generate fallback communication when LLM fails."""

        # Simple template-based fallback
        templates = {
            CommunicationType.CLAIM_STATUS_UPDATE: {
                'subject': f'Claim Status Update - {context.claim_id}',
                'content': f'''Dear {context.customer_name},

We are writing to provide you with an update on your insurance claim {context.claim_id}.

Your claim is currently being processed and we will contact you with additional information as it becomes available.
...
Claims Processing Team'''
            ...
            CommunicationType.APPROVAL_NOTIFICATION: {
                'subject': f'Claim Approved - {context.claim_id}',
                'content': f'''Dear {context.customer_name},

We are pleased to inform you that your claim {context.claim_id} has been approved.

You will receive payment information shortly.
...
Claims Processing Team'''
        ...
        template = templates.get(
            context.communication_type, templates[CommunicationType.CLAIM_STATUS_UPDATE])
    ...
    async def generate_communication_from_assessment(self, assessment_result: Dict[str, Any],
                                                     customer_data: Dict[str, Any]) -> CommunicationResult:
        """
