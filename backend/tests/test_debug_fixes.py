...
async def test_invalid_amount_fix():
    ...
    try:
        ...
        if workflow_state.current_stage == WorkflowStage.FAILED and workflow_state.error_message:
...
async def test_edge_cases():
    ...
    for test_case in test_cases:
        try:
            workflow_state = await orchestrator.process_claim_workflow(test_case['claim'])
...
async def main():
    ...
    try:
        ...
        print(
            f"   Edge Cases: {'✅ PASSED' if edge_cases_pass else '❌ FAILED'}")
