...
class WorkflowTester:
    ...
    async def test_complexity_assessment(self):
        ...
        for test_case in test_cases:
            try:
                ...
                success = (
                    complexity == test_case['expected_complexity'] and
                    should_escalate == test_case['expected_escalation']
                )
    ...
    async def test_custom_workflow(self):
        ...
        for test_case in test_claims:
            try:
                ...
                workflow_state = await self.orchestrator.process_claim_workflow(test_case['claim'])
                end_time = time.time()

                success = (
                    workflow_state.claim_id == test_case['claim']['claim_id'] and
                    workflow_state.current_stage in [WorkflowStage.COMPLETED, WorkflowStage.HUMAN_REVIEW] and
                    len(workflow_state.agent_decisions) > 0
                )
    ...
    async def test_graphflow_workflow(self):
        ...
        try:
            ...
            success = (
                ...
                result.get('workflow_completed', False) and
            ...
            self.log_test_result(
                ...
                success,
                {
                    ...
                    'has_assessment': any('assess' in msg.lower() for msg in result.get('messages', [])),
                    'has_communication': any('dear' in msg.lower() or 'thank you' in msg.lower() for msg in result.get('messages', []))
                }
            )
    ...
    async def test_error_handling(self):
        ...
        for test_case in invalid_claims:
            try:
                ...
                success = (
                    workflow_state.current_stage == WorkflowStage.FAILED or
                    workflow_state.error_message is not None
                )
                ...
                self.log_test_result(
                    ...
                    success,
                    {
                        ...
                        'handled_gracefully': workflow_state.current_stage == WorkflowStage.FAILED
                    }
                )
    ...
    async def test_agent_coordination(self):
        ...
        try:
            ...
            success = (
                assessment_result.get('status') == 'processed' and
                communication_result.get('status') == 'success'
            )
